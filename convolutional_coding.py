...
# (2,1,5)卷积码
# 卷积编码
def encode_conv(image_data):#k为卷积码的约束长度
    x = []
    ...
    registers = [0] * 5
    # 生成多项式
    G1 = [1, 0, 1, 1, 0, 1]  # 对应第一个输出比特
    G2 = [1, 1, 1, 0, 1, 1]  # 对应第二个输出比特
    for j in x:
        c_1 = sum([j * G1[0]] + [registers[k] * G1[k + 1]
                                 for k in range(5)]) % 2
        c_2 = sum([j * G2[0]] + [registers[k] * G2[k + 1]
                                 for k in range(5)]) % 2
        y.append(c_1)
        y.append(c_2)
        # 更新寄存器
...
def decode_conv(y):
    ...
    def encode_with_state(x, state):
        registers = [(state >> i) & 1
                     for i in range(4, -1, -1)]  #将状态转化为二进制列表，存入寄存器
        # 编码后的输出
        y = []
        c_1 = sum([x * G1[0]] + [registers[k] * G1[k + 1]
                                 for k in range(5)]) % 2
        c_2 = sum([x * G2[0]] + [registers[k] * G2[k + 1]
                                 for k in range(5)]) % 2
        y.append(c_1)
        y.append(c_2)
        return y
    ...
    def state_transfer(input, now_state):
        return (
            (now_state >> 1) | (input << 4))  #当前状态右移一位，输入左移6位，然后或运算，得到下一步的状态
    ...
    def update_with_state(y_block, pre_state, index):
        ...
        # 输入为0，且需要更新
        if score_list[pre_state][index] + score_0 < score_list[next_state_0][
                index + 1]:
        ...
        # 输入为1，且需要更新
        if score_list[pre_state][index] + score_1 < score_list[next_state_1][
                index + 1]:  #完成汉明码的对比，得到最小的汉明距离
    ...
    state_trace_index = np.argmin(
        score_list[:, -1]
    )  #拿到最后一列最小值在最后一列中的下标（行号），而在score_list中行号即为状态,即找到最后一个时刻的最小汉明距离对应的状态
    # 记录原编码信息
    x = []
